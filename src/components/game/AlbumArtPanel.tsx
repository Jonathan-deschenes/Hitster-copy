...
export default function AlbumArtPanel({ currentTrack, gameState }: AlbumArtPanelProps) {
	...
	const revealed = manuallyRevealed || gameState?.status === GameStatus.Finished;
	...
	return (
		<div className='relative w-[min(360px,78vw)] shrink-0'>
			...
			>
				...
				{coverImage && !revealed && (
					<div className='absolute inset-0 flex flex-col items-center justify-center gap-2 text-lavender/85 opacity-0 transition-opacity duration-200 group-hover:opacity-100'>
						<span className='flex h-11 w-11 items-center justify-center rounded-full border border-lavender/30 bg-bg-deep/60 backdrop-blur-[6px]'>
							<IconEye />
						</span>
						<span className='text-[0.75rem] font-semibold tracking-[0.03em] uppercase'>
							Révéler
						</span>
					</div>
				)}
				...
				<div className='absolute inset-x-0 bottom-0 z-10 px-5 pb-4 text-left'>
					...
					<h2 className='truncate font-display text-xl font-semibold text-white'>
						{revealed ? (currentTrack?.name ?? "Titre indisponible") : "Titre caché"}
					</h2>
